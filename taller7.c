...
Persona *crearPersona(char *nombre, int edad, int peso){
	Persona *nuevo = (Persona *)malloc(sizeof(Persona)); //*nuevo no estaba inicializado
...
int main(void){
	...
	for(i = 0; i < TAMANO; i++){ //Se estaban creando mas personas de las que se pueden almacenar por la condicion del for
		...
		listaPersona[i] = crearPersona(nombres[i],  edad, peso); //No se estaba agregando a la lista de personas
